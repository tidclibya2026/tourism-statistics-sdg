...
import { arabicNumber, asNumber, axisMeta, formatYear, observationStatusMeta, periodLabel } from "@/lib/tourism";
...
export default function Home() {
  ...
  const activeAreas = useMemo(() => {
    ...
    (spatial.data?.observations ?? []).filter((item) => item.year === latestSpatialYear).forEach((item) => {
      const current = counts.get(item.areaName) ?? { name: item.areaName, type: item.areaType === "city" ? "مدينة" : "إقليم", count: 0 };
      current.count += 1;
      counts.set(item.areaName, current);
    });
    return Array.from(counts.values()).sort((a, b) => b.count - a.count || a.name.localeCompare(b.name, "ar")).slice(0, 5);
  ...
  return (
    <div className="space-y-6" ref={dashboardRef}>
      ...
      {summary.approvedObservations > 0 && <section className="flex flex-col gap-3 rounded-2xl border border-[#b9d9cf] bg-[#f2faf6] px-5 py-4 md:flex-row md:items-center md:justify-between"><div><h2 className="font-bold text-[#173f3d]">نطاق العرض المعتمد</h2><p className="mt-1 text-xs leading-6 text-slate-600">تتغذى هذه اللوحة من القياسات الوطنية المعتمدة فقط{firstCoverageYear && summary.latestYear ? ` للفترة ${formatYear(firstCoverageYear)}–${formatYear(summary.latestYear)}` : ""}. ولا تدخل المسودات أو البيانات الربع سنوية الجزئية في الرسوم والتحليلات النهائية.</p></div><Badge className="w-fit border-0 bg-[#dcefe7] text-[#0f5c58]">{arabicNumber.format(summary.approvedObservations)} قياساً معتمداً</Badge></section>}
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        ...
        <MetricCard label="آخر سنة بيانات" value={summary.latestYear ? formatYear(summary.latestYear) : "—"} icon={CalendarDays} hint="للقياسات السنوية المعتمدة" tone="bg-[#e9f3ed] text-[#2b8062]" />
      ...
      <section className="section-card p-5"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#e9f4f1] text-[#0f766e]"><Activity className="h-5 w-5" /></span><div><h2 className="font-bold text-[#173f3d]">أحدث النشاطات والقياسات</h2><p className="mt-1 text-xs text-slate-500">آخر السجلات المعتمدة الظاهرة وفق فلاتر العرض وصلاحيات حسابك.</p></div></div><Badge variant="outline" className="border-[#b9d7cf] text-[#0f766e]">{arabicNumber.format(data?.recent?.length ?? 0)} سجلات</Badge></div><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">{data?.recent?.length ? data.recent.slice(0, 8).map((item) => <div key={item.observation.id} className="rounded-xl border border-[#e3ece9] bg-[#f9fcfa] p-3"><div className="flex items-start justify-between gap-2"><p className="line-clamp-2 text-sm font-semibold text-[#244844]">{item.indicator.name}</p><Badge className="shrink-0 border-0 bg-[#dff0e9] text-xs text-[#0f5c58]">معتمد</Badge></div><p className="mt-2 text-xs text-slate-500">{formatYear(item.observation.year)} · {periodLabel(item.observation.period, item.observation.quarter)}</p><p className="mt-1 text-base font-bold text-[#0f5c58]">{arabicNumber.format(asNumber(item.observation.value))} <span className="text-xs font-normal text-slate-500">{item.indicator.unit}</span></p></div>) : <p className="rounded-xl border border-dashed border-[#dce8e4] p-5 text-center text-sm text-slate-500">لا توجد نشاطات معتمدة ضمن الفلاتر الحالية.</p>}</div></section>
      ...
      <section className="grid gap-5 xl:grid-cols-[1.4fr_.9fr]">
        <Card className="border-[#dce8e4] shadow-sm"><CardContent className="p-5 md:p-6"><div className="mb-4"><h3 className="font-bold text-[#173f3d]">أحدث القياسات المعتمدة</h3><p className="mt-1 text-xs text-slate-500">تُعرض القياسات ذات حالة الاعتماد فقط؛ لا تظهر المسودات في النسخة النهائية.</p></div>{data?.recent?.length ? <div className="divide-y divide-[#e8efec]">{data.recent.map((item) => <div className="flex items-center justify-between gap-4 py-3" key={item.observation.id}><div className="min-w-0"><p className="truncate text-sm font-semibold text-[#254743]">{item.indicator.name}</p><p className="mt-1 text-xs text-slate-500">{formatYear(item.observation.year)} · {periodLabel(item.observation.period, item.observation.quarter)}</p></div><div className="flex items-center gap-3"><span className="text-sm font-bold text-[#0f5c58]">{arabicNumber.format(asNumber(item.observation.value))} {item.indicator.unit}</span><Badge className={`${observationStatusMeta[item.observation.verificationStatus].className} border-0`}>{observationStatusMeta[item.observation.verificationStatus].label}</Badge></div></div>)}</div> : <EmptyText text="لا توجد قياسات معتمدة ضمن نطاق الفلاتر الحالية." />}</CardContent></Card>
