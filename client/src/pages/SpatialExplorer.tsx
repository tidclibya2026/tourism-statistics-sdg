...
import { Building2, ChevronLeft, Database, MapPinned, Map as MapIcon, RefreshCw, Search, ShieldCheck } from "lucide-react";
...
export default function SpatialExplorer() {
  ...
  }, [data, locationQuickStats, map, visibleCities]);
  ...
  return <div className="space-y-6">
    ...
    {isLoading ? <section className="grid gap-4 md:grid-cols-4">{Array.from({ length: 4 }).map((_, index) => <Skeleton key={index} className="h-28 rounded-2xl" />)}</section> : isError ? <Card className="border-rose-200 bg-rose-50"><CardContent className="flex flex-wrap items-center justify-between gap-3 p-6 text-sm text-rose-700"><span>تعذر تحميل النموذج المكاني حالياً.</span><Button variant="outline" className="border-rose-300 text-rose-700" onClick={() => refetch()}><RefreshCw className="ml-2 h-4 w-4" />إعادة المحاولة</Button></CardContent></Card> : <>
      ...
      <HoverStatisticsGuide locations={matchingLocations} summaries={locationQuickStats} onSelect={(id) => setSelectedArea(String(id))} />
...
function SpatialMapFallback({ cities, onSelect, onRetry }: { cities: { id: number; name: string; parentName: string | null }[]; onSelect: (id: number) => void; onRetry: () => void }) {
