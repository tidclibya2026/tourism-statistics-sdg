...
function SpatialMapFallback({ cities, onSelect, onRetry }: { cities: { id: number; name: string; parentName: string | null }[]; onSelect: (id: number) => void; onRetry: () => void }) {
  ...
  return <div className="relative h-[430px] overflow-hidden bg-[radial-gradient(circle_at_80%_20%,rgba(79,180,172,.25),transparent_24%),linear-gradient(145deg,#f0f7f4,#deece7)]"><div className="absolute inset-x-0 top-[20%] h-20 border-y border-dashed border-[#c0dcd4]/80 bg-white/30" /><div className="absolute left-[7%] top-[8%] rounded-full border border-[#c4ddd5] bg-white/70 px-3 py-1.5 text-xs font-bold text-[#0f766e]">عرض مرجعي بديل</div><Button size="sm" variant="outline" className="absolute right-5 top-5 z-10 border-[#b9d7cf] bg-white/90 text-[#0f766e]" onClick={onRetry}><RefreshCw className="ml-1.5 h-3.5 w-3.5" />إعادة تحميل الخريطة</Button><div className="absolute left-[9%] top-[38%] text-[11px] text-slate-500">البحر المتوسط</div><div className="absolute inset-x-[14%] bottom-[12%] h-24 rounded-[55%_45%_50%_50%] border border-[#c7ddd7] bg-[#eaf3f0] opacity-70" />{cities.map((city) => <button key={city.id} onClick={() => onSelect(city.id)} className={`absolute ${positions[city.name] ?? "right-1/2 top-1/2"} group -translate-x-1/2 -translate-y-1/2 text-center`}><span className="mx-auto grid h-11 w-11 place-items-center rounded-full border-4 border-white bg-[#b98238] text-sm font-bold text-white shadow-[0_8px_20px_rgba(92,63,25,.24)] transition group-hover:scale-110">{city.name.slice(0, 1)}</span><span className="mt-2 block rounded-lg bg-white/90 px-2 py-1 text-xs font-bold text-[#173f3c] shadow-sm">{city.name}</span><span className="mt-1 block text-[10px] text-slate-500">{city.parentName}</span></button>)}<div className="absolute bottom-5 left-5 max-w-xs rounded-xl border border-[#d2e4de] bg-white/85 p-3 text-xs leading-6 text-slate-600">تعذر الاتصال بخدمة الخرائط في بيئة العرض؛ يظل دليل المدن قابلاً للتفاعل ولا يحل محل خريطة حدود إدارية معتمدة.</div></div>;
}
