...
import { BarChart3, FileSpreadsheet, FileText, LineChart as LineChartIcon, LoaderCircle, Printer } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <section className="section-card flex flex-col gap-4 p-5 md:flex-row md:items-end md:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" onClick={exportExcel} disabled={Boolean(exporting) || query.isLoading || !data.length} aria-busy={exporting === "excel"}>
          ...
          <Button className="bg-[#0f5c58] hover:bg-[#0a4845]" onClick={exportPdf} disabled={Boolean(exporting) || query.isLoading || !data.length} aria-busy={exporting === "pdf"}>
      ...
      <div ref={reportRef} className="table-shell bg-white">
        <div className="border-b border-[#e8efec] p-5">
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500">من {formatYear(yearFrom)} إلى {formatYear(yearTo)} · {arabicNumber.format(data.length)} قياس</p>
            </div>
            <div className="flex items-center gap-2 text-[#0f5c58]" aria-label="نوع الرسم التفاعلي">
              ...
              <Button type="button" size="sm" variant={chartMode === "bars" ? "default" : "outline"} className={chartMode === "bars" ? "bg-[#0f5c58] hover:bg-[#0a4845]" : ""} onClick={() => setChartMode("bars")} aria-pressed={chartMode === "bars"}><BarChart3 className="ml-1 h-4 w-4" />أعمدة</Button>
              <Button type="button" size="sm" variant={chartMode === "line" ? "default" : "outline"} className={chartMode === "line" ? "bg-[#0f5c58] hover:bg-[#0a4845]" : ""} onClick={() => setChartMode("line")} aria-pressed={chartMode === "line"}><LineChartIcon className="ml-1 h-4 w-4" />خطي</Button>
            </div>
          </div>
        </div>
        ...
        {annualTrend.length > 0 && (
          <div className="border-b border-[#e8efec] p-5" dir="ltr">
            ...
            <div className="h-56" role="img" aria-label="رسم تفاعلي لعدد القياسات السنوية المعتمدة حسب السنة">
