...
import { Database, Info, Save, Tag } from "lucide-react";
  ...
  return <div className="space-y-6">
    ...
    {canWrite ? <form onSubmit={submit} className="section-card p-5 md:p-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <Field label={`القيمة${selectedIndicator ? ` (${selectedIndicator.unit})` : ""}`}><Input required inputMode="decimal" type="number" step="any" value={form.value} onChange={(event) => setForm((current) => ({ ...current, value: event.target.value }))} /></Field>
        <Field label={`القيمة المستهدفة${selectedIndicator ? ` (${selectedIndicator.unit})` : ""}`}><Input inputMode="decimal" type="number" step="any" value={form.targetValue} onChange={(event) => setForm((current) => ({ ...current, targetValue: event.target.value }))} /></Field>
