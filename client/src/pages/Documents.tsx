...
import { Archive, Download, Eye, FileText, FolderArchive, LockKeyhole, Search } from "lucide-react";
...
export default function Documents() {
  ...
  const audit = trpc.documentAudit?.record?.useMutation?.();
  const [query, setQuery] = useState("");
  const [category, setCategory] = useState<"all" | DocumentCategory>("all");
  const [zipping, setZipping] = useState(false);
  const [preview, setPreview] = useState<(typeof documentLibrary)[number] | null>(null);
  ...
  const previewHtml = useMemo(() => preview ? DOMPurify.sanitize(String(marked.parse(preview.content))) : "", [preview]);

  function recordAudit(action: "document_download" | "documentation_zip_export", outcome: "success" | "failed", resource: string, details?: string) {
    audit?.mutate({ action, outcome, resource, details });
  }
  ...
  function downloadDocument(document: (typeof documentLibrary)[number]) {
    try {
      ...
      recordAudit("document_download", "success", document.fileName, JSON.stringify({ title: document.title, category: document.category }));
      toast.success(`تم تنزيل «${document.title}».`);
    } catch (error) {
      console.error("Documentation download failed", error);
      recordAudit("document_download", "failed", document.fileName);
      toast.error("تعذر تنزيل الدليل حالياً.");
    }
  }
  ...
  function downloadZip() {
    ...
    try {
      ...
      const fileName = `tidc-documentation-${new Date().toISOString().slice(0, 10)}.zip`;
      downloadBlob(new Blob([archive], { type: "application/zip" }), fileName);
      recordAudit("documentation_zip_export", "success", fileName, JSON.stringify({ documentCount: documentLibrary.length }));
      toast.success("تم تنزيل حزمة الأدلة ZIP.");
    } catch (error) {
      console.error("Documentation ZIP export failed", error);
      recordAudit("documentation_zip_export", "failed", "documentation.zip");
  ...
  return <main dir="rtl" className="space-y-6">
    ...
    <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{filtered.map((document) => <Card key={document.fileName} className="flex flex-col border-[#dce8e4] p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#e9f4f1] text-[#0f766e]"><FileText className="h-5 w-5" /></span><Badge className="border-0 bg-[#f4ead9] text-[#8b5d24]">{document.category}</Badge></div><h2 className="mt-4 font-bold text-[#173f3d]">{document.title}</h2><p className="mt-2 flex-1 text-sm leading-7 text-slate-600">{document.description}</p><div className="mt-5 grid grid-cols-2 gap-2"><Button variant="outline" onClick={() => setPreview(document)} className="border-[#b9d7cf] text-[#0f766e]"><Eye className="ml-2 h-4 w-4" />معاينة</Button><Button variant="outline" onClick={() => downloadDocument(document)} className="border-[#b9d7cf] text-[#0f766e]"><Download className="ml-2 h-4 w-4" />تنزيل الدليل</Button></div></Card>)}</section>
    ...
    <Dialog open={Boolean(preview)} onOpenChange={(open) => !open && setPreview(null)}><DialogContent dir="rtl" className="max-h-[88vh] max-w-4xl overflow-y-auto"><DialogHeader><DialogTitle className="text-[#173f3d]">معاينة: {preview?.title}</DialogTitle></DialogHeader><div className="prose prose-slate max-w-none text-right leading-8" dangerouslySetInnerHTML={{ __html: previewHtml }} /></DialogContent></Dialog>
  </main>;
}
